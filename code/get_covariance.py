...
def compute_covariance(station1, station2, stat):
    """
    Compute the covariance between temperature datasets from two CSV files,
    ...
    Returns:
        tuple: (covariance, number of valid samples)
    ...
    if n_samples < 2:
        raise ValueError(f"Only {n_samples} valid sample(s) found; need at least two for covariance")
    ...
    # Compute covariance (sample covariance with ddof=1)
    covariance = np.cov(temp1, temp2, ddof=1)[0, 1]

    return covariance, n_samples

def main():
    """Command-line interface to compute and print covariance between two CSV files."""
    parser = argparse.ArgumentParser(description='Compute covariance between two temperature datasets.')
    ...
    try:
        # Compute covariance and get sample count
        cov, n_samples = compute_covariance(args.station1, args.station2, args.stat)
        print(f"Covariance between {args.station1} and {args.station2} {args.stat} temperature: {cov:.4f}")
