...
% \subsection{Time-Space Analysis of $VAR(p)$ Model Parameters}
% In this subsection we briefly investigate room for model improvement by including the effects of other weather phenomena.
% \subsubsection{El-Ni\tilde{n}o Southern Oscillation Effects}
% The El-Ni\tilde{n}o southern oscillation (ENSO) is an oscillatory southern hemisphere weather phenomenon. It is measured using the Southern Oscillation Index (SOI), given by normalised pressure difference between Darwin and Tahiti. The American NOAA (National Oceanic and Atmospheric Administration) reports a monthly SOI score \cite{noaa}. ENSO is one of the major drivers of south hemisphere weather dynamics.\\

% We use Lasso regression with $\lambda=0.4$ on a year-long monthly rolling window to perform time-domain analysis of $VAR(1)$ transition matrix coefficients. Fig. \ref{fig:enso} shows SOI against normalised variation in some matrix coefficients.
% \begin{figure}[!ht]
%     \centering
%     \includegraphics[width=.7\linewidth]{}
%     \caption{$VAR(1)$ select normalised coefficents vs ENSO over time.}
%     \label{fig:enso}
% \end{figure}
