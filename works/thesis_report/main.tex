...
\begin{align*}
    p&=\sum_i^N\frac{\partial \hat{y_i}}{\partial y_i}\\
    &=\sum_i^N\frac{\partial}{\partial y_i}\pmb{X}(\pmb{X}^\top \pmb{X})^{-1}\pmb{X}^\top y_i\\
    &=\text{tr}(\pmb{X}(\pmb{X}^\top \pmb{X})^{-1}\pmb{X}^\top)\\
    &=\text{tr}(\pmb{X})\\
    &=N
\end{align*}
This is exactly the number of effective parameters we expect. $H$ is called the smoothing matrix, that takes the sample output to the predicted output. For ridge regression, the smoothing matrix is as below.
\begin{align*}
